...
export default function LeadDetails() {
  ...
  const actorIds = useMemo(() => {
    ...
    (audit || []).forEach((a) => {
      if (a.actor_id) s.add(a.actor_id);
    });
    return Array.from(s);
  }, [lead, audit]);
  const { data: actors } = useQuery({
    queryKey: ["users-lookup", actorIds.join(",")],
    queryFn: () =>
      actorIds.length ? UsersAPI.lookup(actorIds) : Promise.resolve([]),
    ...
    <div className="space-y-6 animate-in-up">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          {lead.is_overdue && (
            <div className="inline-flex mt-2 px-2 py-1 rounded-full bg-rose-100 text-rose-700 text-xs font-medium">
              Overdue Followup
            </div>
          )}
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Panel title="Lead Info">
          <div className="space-y-2 text-sm">
            ...
            <Row label="Stage">
              {lead.pipeline_stage ? (
                <Badge value={lead.pipeline_stage} />
              ) : (
            ...
            <Row label="Expected Value">
              <span className="font-medium">
                {(lead.expected_value ?? 0).toLocaleString("en-GB")}
              </span>
            </Row>
            <Row label="Source">
            ...
            <Row label="Assigned To">
              <span>
                {lead.assigned_to ? nameOf(lead.assigned_to) : "UNASSIGNED"}
              </span>
            </Row>
            <Row label="Assigned By">
              <span>{lead.assigned_by ? nameOf(lead.assigned_by) : "-"}</span>
            </Row>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(lead.tags || []).map((t, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-brand-50 text-brand-700 border border-brand-100"
                  >
                    {t}
                    <button
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Panel title={`Followups (${(followups || []).length})`}>
          <div className="space-y-3">
            {(followups || []).map((f) => (
              <div key={f._id} className="border rounded-xl p-3">
                <div className="flex items-center justify-between">
                  ...
                  {f.outcome && (
                    <div className="text-xs text-slate-500">{f.outcome}</div>
                  )}
                </div>
            ...
            {(followups || []).length === 0 && (
              <div className="text-sm text-slate-500">No followups yet.</div>
            )}
          </div>
        </Panel>
      ...
      <FollowupModal
        open={!!openFU}
        onClose={() => setOpenFU(null)}
        onSubmit={(payload) => addFollowup.mutate(payload)}
        products={products || []}
        leadId={id}
        qc={qc}
        productId={openFU?.productId}
      />
      <ProductModal
        open={openProduct}
...
function NextActions({ lead, onChange, onProb }) {
  ...
  const toggle = (idx) => {
    const arr = (lead.next_actions || []).map((a, i) =>
  ...
  return (
    <div className="space-y-3 text-sm">
      <div className="flex gap-2">
        ...
        />
        <button onClick={add} className="px-3 py-2 rounded-lg border text-sm">
          Add
        </button>
      </div>
  ...
  return (
    <div className="mt-3">
      <div className="flex gap-2">
        ...
        />
        <button className="px-3 py-2 rounded-lg border text-sm" onClick={add}>
          Add note
        </button>
      </div>
...
function TagAdder({ onAdd }) {
  ...
  return (
    <div className="inline-flex items-center gap-2">
      ...
      <button
        onClick={add}
        className="px-2 py-1 rounded bg-brand-600 text-white text-xs hover:bg-brand-700"
      >
        Add
      </button>
    </div>
...
function FollowupModal({
  ...
  const submit = () => {
    ...
    if (selectedProductId) {
      ...
      LeadsAPI.addProductFollowup(leadId, selectedProductId, payload)
        .then(() => {
          ...
          >
            <Dialog.Panel className="w-full max-w-xl bg-white rounded-2xl shadow-card border p-5">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  <div className="text-xs text-slate-500 flex justify-between">
                    <span>Next Followup At (optional)</span>
                    {nextAt && (
                      <span className="text-[11px] text-slate-400">
                        {new Date(nextAt).toLocaleTimeString("en-GB", {
                          hour: "2-digit",
                          minute: "2-digit",
                          hour12: true,
                        })}
          ...
          >
            <Dialog.Panel className="w-full max-w-xl bg-white rounded-2xl shadow-card border p-5">
              ...
              <div className="mt-4 flex justify-end gap-2">
                ...
                <button
                  onClick={submit}
                  className="px-3 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 text-sm shadow-soft hover:shadow-hover"
                >
                  Add
                </button>
              </div>
