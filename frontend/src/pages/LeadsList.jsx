    ...
    <div className="space-y-4 animate-in-up">
              ...
              <tr>
                <th className="py-3 px-4">
                  <input
                    type="checkbox"
                    onChange={(e) => {
                      e.target.checked ? selectAllPage() : clearSelected();
                    }}
                  />
                </th>
                <th
                  ...
                  onClick={() => {
                    setSortBy("name");
                    setSortDir(sortBy === "name" ? -sortDir : 1);
                  }}
                >
                ...
                <th
                  ...
                  onClick={() => {
                    setSortBy("status");
                    setSortDir(sortBy === "status" ? -sortDir : 1);
                  }}
                >
                  Status{" "}
                  {sortBy === "status" ? (sortDir === 1 ? "▲" : "▼") : ""}
                </th>
                <th
                  className="px-4 cursor-pointer select-none"
                  onClick={() => {
                    setSortBy("temperature");
                    setSortDir(sortBy === "temperature" ? -sortDir : 1);
                  }}
                >
                  Temp{" "}
                  {sortBy === "temperature" ? (sortDir === 1 ? "▲" : "▼") : ""}
                </th>
                <th
                  className="px-4 cursor-pointer select-none"
                  onClick={() => {
                    setSortBy("pipeline_stage");
                    setSortDir(sortBy === "pipeline_stage" ? -sortDir : 1);
                  }}
                >
                  Stage{" "}
                  {sortBy === "pipeline_stage"
                    ? sortDir === 1
                      ? "▲"
                      : "▼"
                    : ""}
                ...
                <th
                  className="px-4 cursor-pointer select-none"
                  onClick={() => {
                    setSortBy("next_followup_at");
                    setSortDir(sortBy === "next_followup_at" ? -sortDir : 1);
                  }}
                >
                  Next Followup{" "}
                ...
                <th
                  ...
                  onClick={() => {
                    setSortBy("expected_value");
                    setSortDir(sortBy === "expected_value" ? -sortDir : 1);
                  }}
                >
                  Value{" "}
              ...
              {(data?.items || []).map((l) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    ...
                    {l.is_overdue && (
                      <div className="text-xs text-rose-700 mt-1">Overdue</div>
                    )}
                  </td>
                  <td className="px-4">
      ...
      <BulkTempModal
        ...
        onConfirm={async (temperature) => {
          try {
            await LeadsAPI.bulkTemperature({
              lead_ids: Array.from(selected),
              temperature,
            });
          ...
          >
            <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl shadow-card border p-5">
              ...
              <div className="mt-3 text-sm">
                Assign selected leads to a sales user. Choose empty to unassign.
              </div>
              <div className="mt-3">
                <select
                  className="w-full"
              ...
              <div className="mt-4 flex justify-end gap-2">
                ...
                <button
                  onClick={confirm}
                  className="px-3 py-2 rounded-lg bg-brand-600 text-white hover:bg-brand-700 text-sm"
                >
                  Confirm
                </button>
              </div>
          ...
          >
            <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl shadow-card border p-5">
              ...
              <div className="mt-4 text-xs text-slate-500">
                This will update status for all selected leads.
              </div>
              <div className="mt-4 flex justify-end gap-2">
                <button
                  onClick={onClose}
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl shadow-card border p-5">
              ...
              <div className="mt-4 text-xs text-slate-500">
                This will update temperature for all selected leads.
              </div>
              <div className="mt-4 flex justify-end gap-2">
                <button
                  onClick={onClose}
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl shadow-card border p-5">
              ...
              <div className="mt-4 text-xs text-slate-500">
                This will update pipeline stage for all selected leads.
              </div>
              <div className="mt-4 flex justify-end gap-2">
                <button
                  onClick={onClose}
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            <Transition.Child
              ...
              leaveTo="opacity-0 translate-y-2 scale-95"
            >
              <Dialog.Panel className="w-full max-w-md rounded-2xl border bg-white p-5 shadow-card">
                <Dialog.Title className="text-lg font-semibold">
                  Bulk CSV Import
                </Dialog.Title>
          ...
          >
            <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl shadow-card border p-5">
              ...
              <div className="mt-3">
                <input
                  className="w-full"
                  placeholder="View name (e.g., My Overdue)"
                  value={name}
